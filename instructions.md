...
- **Jitsi Meet** *(recommended for StartOS self-hosters).* Cal.diy's Jitsi app is preinstalled; with no further configuration, meeting links default to the public `https://meet.jit.si/cal` server. To keep everything local: install the **Jitsi** package from the StartOS marketplace (it's listed as an optional companion to Cal.diy in the marketplace), then run Cal.diy's **Set Up Self-Hosted Jitsi** action — it pulls Jitsi's URLs straight from StartOS, lets you pick which one to use for booking links, and updates Cal.diy's app config in one step. No need to copy/paste URLs between StartOS Dashboard and Cal.diy's admin UI.
